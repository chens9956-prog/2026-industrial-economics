...
# 1. Update index.wxml to include DEG/RAD mode switcher
...
# 2. Update index.js with Auto-Closing Parentheses & DEG Mode (sin(30)=0.5, sin(90)=1)
index_js = """Page({
  data: {
    expression: '',
    mainDisplay: '0',
    subDisplay: '',
    isSciMode: true,
    angleMode: 'DEG', // DEG (角度) or RAD (弧度)
    showHistory: false,
    historyList: [],
  ...
  evalExpression(expr) {
    ...
    // 1. Auto-close missing right parentheses ')'
    ...
    const isDeg = this.data.angleMode === 'DEG';
    ...
    if (isDeg) {
      // Convert sin(x) -> Math.sin((x) * Math.PI / 180) for Degree mode
      sanitized = sanitized
    ...
    try {
      ...
      // Clean up tiny floating point precision artifacts e.g. sin(180) -> 0
      if (typeof val === 'number' && Math.abs(val) < 1e-12) val = 0;
...
# 3. Update index.wxss
index_wxss = """/* ==========================================================================
   TRIG FUNCTIONS & ANGLE MODE (DEG/RAD) SUPPORT
   ========================================================================== */
...
.tab-left { display: flex; gap: 16rpx; }
.angle-btn {
  font-size: 20rpx;
  font-weight: bold;
  color: #f59e0b;
...
print("Fixed trig function evaluation, auto-closed parentheses, and added DEG/RAD switcher successfully!")
