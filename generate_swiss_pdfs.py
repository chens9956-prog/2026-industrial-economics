import os
import json
import re
from docx import Document
from docx.shared import Pt, Inches, RGBColor
from docx.enum.text import WD_ALIGN_PARAGRAPH
import win32com.client
from docx.oxml.ns import qn

# Swiss Design Colors
SWISS_RED = RGBColor(227, 0, 15) # Classic Swiss Design Red #E3000F
SWISS_BLACK = RGBColor(0, 0, 0) # High contrast black
SWISS_GRAY = RGBColor(80, 80, 80) # Dark gray for subtext

def set_run_font(run, font_name, font_size, color, bold=False):
    run.font.name = font_name
    run.font.size = Pt(font_size)
    run.font.bold = bold
    run.font.color.rgb = color
    # Extremely important for Chinese text in python-docx to avoid fallback font changes
    run.font.element.rPr.rFonts.set(qn('w:eastAsia'), font_name)

def extract_answers():
    transcript_path = r"C:\Users\ausu\.gemini\antigravity\brain\48a8be4f-7705-48d3-9456-30d2f3f49048\.system_generated\logs\transcript.jsonl"
    with open(transcript_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        
    ans_list = []
    for line in lines:
        try:
            data = json.loads(line)
            if data.get('type') == 'MCP_TOOL':
                content = data.get('content', '')
                ans = ''
                if '{"status":"success"' in content and '"answer"' in content:
                    ans = json.loads(content[content.find('{"status":"success"'):]).get('answer', '')
                elif 'The output was large and was saved to: file://' in content:
                    file_path = content.split('file:///')[1].strip().replace('/', '\\')
                    if os.path.exists(file_path):
                        with open(file_path, 'r', encoding='utf-8') as f:
                            file_content = f.read()
                            if '{"status":"success"' in file_content and '"answer"' in file_content:
                                ans = json.loads(file_content[file_content.find('{"status":"success"'):]).get('answer', '')
                if ans:
                    ans_list.append(ans)
        except:
            pass

    answers = {}
    last_10 = ans_list[-10:]
    for i, a in enumerate(last_10):
        answers[i + 1] = a
    return answers

def create_pdfs():
    output_dir = r"I:\4产业经济学\瑞士设计_要点PDF"
    os.makedirs(output_dir, exist_ok=True)
    
    answers = extract_answers()
    
    if not answers:
        print("未找到任何摘要结果。")
        return
        
    print(f"找到 {len(answers)} 个章节的摘要。")
        
    word = win32com.client.Dispatch("Word.Application")
    word.Visible = False

    for ch_num, answer in answers.items():
        try:
            doc = Document()
            
            # Setup Page layout (A4)
            section = doc.sections[0]
            section.page_width = Inches(8.27)
            section.page_height = Inches(11.69)
            # Ample whitespace is a hallmark of Swiss design, but we must fit in 1 page.
            # We'll use asymmetric left margin if possible, but keep it standard for A4 PDF fit.
            section.top_margin = Inches(1)
            section.bottom_margin = Inches(0.8)
            section.left_margin = Inches(1.5)  # Slightly larger left margin for strong vertical alignment
            section.right_margin = Inches(1.2)
            
            ch_str = f"CH{ch_num:02d}"
            
            # Header
            p = doc.add_paragraph()
            # Swiss design relies heavily on flush-left (left-aligned) typography
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT
            
            # Subtitle (Subject)
            r_sub = p.add_run("产业经济学\n")
            set_run_font(r_sub, 'Microsoft YaHei', 14, SWISS_GRAY, bold=False)
            
            # Main Title (Chapter)
            r_main = p.add_run(f"{ch_str} 核心精粹")
            # Swiss design uses bold heavy stark typography for main headers
            set_run_font(r_main, 'Microsoft YaHei', 36, SWISS_BLACK, bold=True)
            
            p.paragraph_format.space_after = Pt(24)
            
            points = []
            for p_text in answer.split('\n'):
                p_text = p_text.strip()
                if p_text:
                    clean_p = p_text.replace("**", "")
                    # Strip citations
                    clean_p = re.sub(r'\[\s*\d+(?:\s*,\s*\d+)*\s*\]', '', clean_p)
                    points.append(clean_p)
            
            for point in points:
                m = re.match(r"^(\d+\.)\s*(.*)", point)
                
                para = doc.add_paragraph()
                para.paragraph_format.space_after = Pt(12)
                para.paragraph_format.line_spacing = 1.2
                para.alignment = WD_ALIGN_PARAGRAPH.LEFT
                
                base_size = 11
                
                if m:
                    num_str = m.group(1)
                    content_str = m.group(2)
                    
                    # Stark Red for the numbering to provide that classic Swiss contrast
                    r_num = para.add_run(num_str + " ")
                    set_run_font(r_num, 'Microsoft YaHei', base_size + 1, SWISS_RED, bold=True)
                    
                    r_content = para.add_run(content_str)
                    set_run_font(r_content, 'Microsoft YaHei', base_size, SWISS_BLACK, bold=False)
                else:
                    r_content = para.add_run(point)
                    set_run_font(r_content, 'Microsoft YaHei', base_size, SWISS_BLACK, bold=False)
                    
            docx_filename = f"产业经济学{ch_str}_要点汇编_瑞士设计.docx"
            pdf_filename = f"产业经济学{ch_str}_要点汇编_瑞士设计.pdf"
            
            docx_path = os.path.join(output_dir, docx_filename)
            pdf_path = os.path.join(output_dir, pdf_filename)
            
            doc.save(docx_path)
            
            abs_docx_path = os.path.abspath(docx_path)
            abs_pdf_path = os.path.abspath(pdf_path)
            
            word_doc = word.Documents.Open(abs_docx_path)
            word_doc.SaveAs(abs_pdf_path, FileFormat=17)
            word_doc.Close()
            
            os.remove(abs_docx_path)
            
            print(f"Successfully generated {pdf_filename}")
            
        except Exception as e:
            print(f"Failed to process CH{ch_num:02d}: {str(e)}")

    word.Quit()
    print("All PDFs processed.")

if __name__ == "__main__":
    create_pdfs()
